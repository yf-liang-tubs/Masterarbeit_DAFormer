...
def process_data_not_training(per, niter,dataset, layer):
    pixels_reduced_2d = torch.load(f'/home/yliang/work/DAFormer/save_file/before_training_feature/visiualize/per{per}_iter{niter}/{layer}_{dataset}_tensor.pt')
    selected_labels = np.load(f'/home/yliang/work/DAFormer/save_file/before_training_feature/visiualize/per{per}_iter{niter}/{layer}_{dataset}_selected_label.npy')
...
# 使用函数处理不同的数据集和特征
selected_labels_1, x_1, y_1, point_colors_1 = process_data_not_training('40', '1500',dataset='CS',layer='f4')
selected_labels_2, x_2, y_2, point_colors_2 = process_data_not_training('40', '1500',dataset='GTA5',layer='f4')

selected_labels_3, x_3, y_3, point_colors_3 = process_data(per, niter, model='HRDA_gta', layer='f4', dataset='cityscapes')
selected_labels_4, x_4, y_4, point_colors_4 = process_data(per, niter, model='HRDA_gta', layer='f4', dataset='gta5')
