...
hrda_synthia_decode_encode=hrda_decode_encode.to(device)
hrda_synthia_decode_encode.eval()
...
def activate_and_extract_features(model, dataloader, target_layer_names):
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            img_metas = {key: value.to(device) for key, value in img_metas.items()} #other
...
layer_names = []
for name, module in hrda_synthia_decode_encode.named_modules():
    print(name)
    if 'fuse_layer.bottleneck.bn' in name:
        layer_names.append(name)
# print(layer_names)

dataset_dataloader = synthia_dataloader  #cs_dataloader/gta5_dataloader

extracted_features_HDRA = activate_and_extract_features(hrda_synthia_decode_encode, dataset_dataloader, layer_names)
torch.save(extracted_features_HDRA, f'/home/yliang/work/DAFormer/save_file/encode_decode_feature/HRDA_synthia/synthia_decode_head.fuse_layer.bottleneck.bn_batch500.pt')
